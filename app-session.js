...
(function () {
  ...
  function installAndroidHomeExitControl() {
    if (!isHomePath(location.pathname) || !document.documentElement) return;
    ...
    var MODAL_ID = "efp-android-exit-modal";

    if (!document.getElementById(STYLE_ID) && document.head) {
      var style = document.createElement("style");
      style.id = STYLE_ID;
      style.textContent = [
        "html.efp-android-exit-home .youtube-banner{padding-left:52px!important;padding-right:52px!important}",
        "#efp-android-exit-button{position:fixed;top:5px;right:max(7px,env(safe-area-inset-right));z-index:2147483646;width:34px;height:34px;display:grid;place-items:center;border:1px solid rgba(248,113,113,.34);border-radius:10px;background:rgba(127,29,29,.16);color:#fecaca;box-shadow:0 4px 14px rgba(0,0,0,.22);font:800 22px/1 system-ui,-apple-system,'Segoe UI',sans-serif;cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:manipulation;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}",
        "#efp-android-exit-button:active{transform:scale(.94);background:rgba(185,28,28,.30)}",
        "#"+MODAL_ID+"{position:fixed;inset:0;z-index:2147483647;display:none;align-items:center;justify-content:center;padding:18px;background:rgba(3,7,18,.70);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)}",
        "#"+MODAL_ID+".show{display:flex}",
        "#"+MODAL_ID+" .efp-exit-card{width:min(390px,100%);border:1px solid rgba(255,255,255,.10);border-radius:20px;background:#111827;color:#f8fafc;box-shadow:0 28px 80px rgba(0,0,0,.42);overflow:hidden}",
        "#"+MODAL_ID+" .efp-exit-body{padding:23px 22px 17px;text-align:center}",
        "#"+MODAL_ID+" .efp-exit-icon{width:46px;height:46px;margin:0 auto 13px;display:grid;place-items:center;border-radius:14px;border:1px solid rgba(248,113,113,.34);background:rgba(127,29,29,.20);color:#fca5a5;font-size:24px;font-weight:900}",
        "#"+MODAL_ID+" h2{margin:0 0 7px;color:#fff;font:800 19px/1.25 system-ui,-apple-system,'Segoe UI',sans-serif}",
        "#"+MODAL_ID+" p{margin:0;color:#cbd5e1;font:500 13px/1.5 system-ui,-apple-system,'Segoe UI',sans-serif}",
        "#"+MODAL_ID+" .efp-exit-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px;padding:14px 18px 18px;border-top:1px solid rgba(255,255,255,.08)}",
        "#"+MODAL_ID+" button{min-height:44px;border-radius:11px;font:800 13px/1 system-ui,-apple-system,'Segoe UI',sans-serif;cursor:pointer}",
        "#"+MODAL_ID+" .efp-exit-stay{border:1px solid #475569;background:#1f2937;color:#f8fafc}",
        "#"+MODAL_ID+" .efp-exit-confirm{border:1px solid #ef4444;background:#b91c1c;color:#fff}",
        "@media(max-width:430px){html.efp-android-exit-home .youtube-banner .yt-cta{display:none!important}#"+MODAL_ID+" .efp-exit-actions{grid-template-columns:1fr}}"
      ].join("");
      document.head.appendChild(style);
    }

    document.documentElement.classList.add("efp-android-exit-home");

    var button = document.createElement("button");
    button.id = "efp-android-exit-button";
    button.type = "button";
    button.setAttribute("aria-label", "Exit ExamFusion Prep");
    button.setAttribute("title", "Exit app");
    ...
    function ensureModal() {
      ...
      modal.innerHTML =
        '<div class="efp-exit-card">' +
          '<div class="efp-exit-body">' +
            '<div class="efp-exit-icon" aria-hidden="true">&times;</div>' +
            '<h2 id="' + MODAL_ID + '-title">Do you really want to exit?</h2>' +
            '<p>Your progress is already saved.</p>' +
          '</div>' +
          '<div class="efp-exit-actions">' +
            '<button type="button" class="efp-exit-stay">Stay</button>' +
            '<button type="button" class="efp-exit-confirm">Exit App</button>' +
          '</div>' +
        '</div>';
    ...
    button.addEventListener("click", function (event) {
      ...
      var stay = modal.querySelector(".efp-exit-stay");
      var confirm = modal.querySelector(".efp-exit-confirm");
      modal.classList.add("show");

      stay.onclick = function () { closeModal(modal); };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", installAndroidHomeExitControl, { once: true });
  } else {
    installAndroidHomeExitControl();
  }
