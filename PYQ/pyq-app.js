(() => {
  ...
  function renderStats(){
    ...
    els.stats.innerHTML = [
      `${state.filtered.length.toLocaleString()} questions`,
  ...
  function optionHtml(q, i, text){
    return `<button class="opt" data-q="${esc(q.id)}" data-i="${i}">${String.fromCharCode(65+i)}. ${esc(text)}</button>`;
  }
  ...
  function render(){
    ...
    els.list.innerHTML = state.filtered.map((q, idx) => {
      ...
      return `<article class="question" data-id="${esc(q.id)}">
        <div class="qmeta">${idx+1}. ${esc(meta)}</div>
        ...
        <div class="source">Source: ${srcHtml}</div>
      </article>`;
    }).join('');
  }
  ...
  function applySearch(){
    ...
    if (!term) state.filtered = state.questions.slice();
      ...
      q.question, q.question_hi, ...(q.options||[]), ...(q.options_hi||[]),
  ...
  function bindAnswers(){
    els.list.addEventListener('click', e => {
      ...
      const q = state.questions.find(x => String(x.id) === b.dataset.q); if (!q) return;
      ...
      if (Number(b.dataset.i) !== Number(q.answer)) b.classList.add('wrong');
